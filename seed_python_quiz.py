...
def auto_seed_python():
    print("🚀 Initializing Python 200-Question Target Seeder Engine...")
    
    subject, _ = Subject.objects.get_or_create(name="Python")
    total_questions_added = 0
    
    # Grab the nested Python questions from our consolidated data pool
    python_questions = DATA_POOL.get("Python", {})

    for topic_name, base_questions in python_questions.items():
        print(f"\nSeeding Topic: [{topic_name}]")
        
        # We loop to generate a robust question pool matching the dropdowns
    ...
    print(f"\n🎉 Success! Added {total_questions_added} synced Python questions into the database.")
